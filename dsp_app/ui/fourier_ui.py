...
def fourier_tab(display_mode):
    ...
    # Compute DFT
    X = smart_dft_idft(samples, inverse=False)
    ...
    if expected_file is not None:
        ...
        amp_for_test = [round(a, 3) for a in amp]
        ph_for_test  = [round(p, 3) for p in ph]

        expected_amp_r = [round(a, 3) for a in expected_amp]
        expected_phase_r = [round(p, 3) for p in expected_phase]
    ...
    if st.button("Reconstruct using IDFT"):
        rec = smart_dft_idft(X, inverse=True)
